...
def non_max_suppression(
    ...
    nm = prediction.shape[1] - nc - 4     # Mask 개수 predict할 땐 0
    mi = 4 + nc  # mask start index
    xc = prediction[:, 4:mi].amax(1) > conf_thres  # candidates (B,8400)
    ...
    prediction = prediction.transpose(-1, -2)  # shape(1,Class_num+4,8400) to shape(1,8400,Class_num+4) 4는 xywh
    img_vec = img_vec.transpose(-1, -2)        # shape(1,128,8400) to (1,8400,128)
    
    prediction[..., :4] = xywh2xyxy(prediction[..., :4])  # xywh to xyxy

    t = time.time()
    output = [torch.zeros((0, 6 + nm), device=prediction.device)] * bs
    img_vec_output = [torch.zeros((0, 128), device=prediction.device)] * bs ############ 여기 수정

    for xi, x in enumerate(prediction):  # image index, image inference , Batch만큼 반복
        # Apply constraints
        # x[((x[:, 2:4] < min_wh) | (x[:, 2:4] > max_wh)).any(1), 4] = 0  # width-height
        
        x = x[xc[xi]]  # confidence , 8400개의 후보 박스 중 thres를 넘긴 박스만 남긴다. ex) (31,class_num+4)
        img_vec_in = img_vec[xi][xc[xi]]                                                                    #  ex) (31,128)
        ...
        if multi_label:
            ...
            conf, j = cls.max(1, keepdim=True)  # 값 , 인덱스
            x = torch.cat((box, conf, j.float(), mask), 1)[conf.view(-1) > conf_thres]
            img_vec_in = img_vec_in[conf.view(-1) > conf_thres]
        ...
        if n > max_nms:  # excess boxes
            img_vec_in = img_vec_in[x[:, 4].argsort(descending=True)[:max_nms]]
        ...
        output[xi] = x[i]
        img_vec_output[xi] = img_vec_in[i]
