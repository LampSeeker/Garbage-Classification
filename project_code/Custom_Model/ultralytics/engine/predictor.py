...
class BasePredictor:
    ...
    def inference(self, im, *args, **kwargs):
        ...
        result , img_vec = self.model(im, augment=self.args.augment, visualize=visualize)
        return result , img_vec
    ...
    def stream_inference(self, source=None, model=None, *args, **kwargs):
        ...
        for batch in self.dataset:
            ...
            self.batch = batch
            path, im0s, vid_cap, s = batch  # s는 이미지type (ex텐서)
            ...
            # Inference
            with profilers[1]:
                preds , img_vec = self.inference(im, *args, **kwargs)

            # Postprocess
            with profilers[2]:
                self.results , img_vec_output = self.postprocess(preds, img_vec ,im, im0s)
            ...
            yield from (self.results , img_vec_output ,preds)
